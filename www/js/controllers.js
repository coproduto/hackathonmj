...
function getConvenios(map, addressData, consultaSiconv, geocoding, $ionicLoading, callback) {
    ...
    consultaSiconv.municipios({nome: addressData.city}, function(cityData) {
	var cityId = cityData.municipios[0].id;
	consultaSiconv.proponentes({id_municipio: cityId}, function(propData) {
	    var props = propData.proponentes;
	    var propIds = props.map(function(prop) { return prop.id });
	    console.log(propIds.slice(0,10));
	    dadosConvenio.endereco = props[0].endereco

	    consultaSiconv.convenios({id_proponente: propIds[0]}, function(convData) {
		...
		geocoding.get({address: dadosConvenio.endereco + ', ' + addressData.city + ', ' + addressData.state}, 
                   function(geoData) {
